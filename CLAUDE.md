...
- **12 actions in manifest** (10 button + 2 knob): play-pause, next, previous, mute, preset, group-preset, repeat, shuffle, input-select, profile-switch, volume, group-volume. Original 6 were defined in Phase 1; the rest added in Phases 6-7.
...
- **PI communication via global settings:** The PI communicates with the plugin exclusively through `setGlobalSettings`/`didReceiveGlobalSettings` -- NOT `sendToPlugin`/`sendToPropertyInspector` (VSD Craft may not support those). PI saves `heosIp`/`playerId` to trigger plugin actions. Plugin saves `_piPlayers`, `_piGroups`, `_piSources`, `_piSignedIn`, `_piError`, and `_piDiscoveryResults` to global settings. PI reads these fields in `didReceiveGlobalSettings` to populate dropdowns, show status, and display SSDP discovery results. SSDP discovery is triggered by PI setting `_discoverRequest: Date.now()`.
- **Speaker profiles** stored in global settings as `profiles: [{ id, name, ip, playerId }]`. Profile switching updates `heosIp`/`playerId` which triggers the existing IP-change detection and reconnection flow -- no separate connection management needed.
